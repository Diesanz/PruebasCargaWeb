...
\chapter{Página web}
\section{Descripción de nuestra página web}
\textbf{Descripción}:En EATTHIS!!, somos un equipo joven con ganas de revolucionar la forma en la que comes. Apostamos por una alimentación equilibrada, sin renunciar al sabor ni a la comodidad. Ofrecemos platos saludables, ricos, nutritivos y adaptados a tus objetivos, así como la posibilidad de preparate tus propios menus con ingredientes de calidad para que cuidar de ti sea fácil y apetecible.

\textbf{Funcionalidad}:El contenido y funcionalidad se detalla
más adelante
    ...
    \begin{enumerate}
        ...
        \item La carpeta templates está formada por:
            \begin{itemize}
                ...
                \item \textbf{pedidos.html} $\rightarrow$ representa la vista del historial de pedidos del usuario. En esta vista se utiliza la paginación para poder visualizar de poco en poco los pedidos de un usuario y no sobrecargar la ase de datos.
...
\section{API}
    ...
    \subsection{Endpoints de la práctica}
        \begin{itemize}
            ...
            \begin{itemize}
                \item \textbf{GET /api/pedidos} $\rightarrow$ devuelve la lista de pedidos del usuario autenticado. Como se ha dicho anteriormente este endpoint trabaja con un paginación de pedidos, para no sobrecargar la base de datos en caso de que existan demasiados pedidos.
...
\subsection{Archivo \texttt{run.py}}
    ...
    if __name__ == '__main__':
        app.run(host='10.0.31.22',debug=True)
    \end{lstlisting}
    ...
    \begin{itemize}
      ...
      \item Inicia el servidor en modo \texttt{debug}. En producción el modo \texttt{debug = False} 
        \item En este caso, con el host 10.0.31.22, la aplicación se está exponiendo en esa IP específica de la red local, esto permite que otros dispositivos de la misma red puedan acceder a la app.
    \end{itemize}
...
\end{itemize}
Este patrón de verificación del token se ha aplicado de forma consistente en otros puntos de la aplicación donde se requiere autenticación como agreagar un producto al carrito, visualizar los pedidos, etc. \\

De este modo se asegura la integridad de la web y facilita tareas de seguridad.

\subsection{Ejemplo del tratamiento de la paginación en nuestra web}
Con esta función se busca reducir la cantidad de pedidos que puede visualizar un usuario de forma simultánea. El objetivo principal de esta paginación es mejorar el rendimiento del sistema, evitando sobrecargar la interfaz de usuario y la base de datos con demasiada información en una sola consulta.

La paginación permite mostrar los pedidos del usuario en bloques limitados, cargando únicamente los elementos necesarios según la página solicitada. Esto es especialmente útil cuando el número de pedidos es alto.

\begin{lstlisting}[style=pythonclean, caption={Obtener pedidos de un usuario}]
...
\subsection{Conexión a la base de datos}
    ...
    \begin{enumerate}
        ...
        \end{itemize}
        \item \textbf{Cargar toda la base de datos}: Si se quisiera cargar toda la base de datos tablas, procedimientos y población de datos, habría que ejecutar  \lstinline{source /home/usuario/esi/PruebasCargawev/srcips_db/carga_web_dump.sql;}
...
\subsubsection{Obtener los pedidos de un usuario (GET /api/pedidos)}
\begin{lstlisting}[style=pythonclean, caption={Tarea para obtener el menú de productos}]
@task(1)
...
\end{lstlisting}
La siguiente función tiene como objetivo simular una petición GET al endpoint que devuelve los pedidos de un usuario.Está diseñada para simular el comportamiento real de un usuario navegando por sus páginas de pedidos.

Como los pedidos se cargan mediante paginación (para evitar sobrecargar la base de datos), se selecciona aleatoriamente una página de pedidos en cada ejecución de la función. Si la lista de pedidos (`self.lista\_pedidos`) ya ha sido obtenida previamente, se calcula el número total de páginas y se accede aleatoriamente a una de ellas. Si no se tiene información previa, se accede directamente a la primera página por defecto.
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item Como detalle se destaca que el servidor cuenta con dos núcleos.
    \end{itemize}
...
\begin{itemize}
   ...
   \item En total se han capturado 66\,318 peticiones, de las cuales 11\,225 han fallado, lo que representa una tasa de error del 16,9\%. El tiempo de respuesta medio ha sido de 149{,}12 ms, con un mínimo de 4 ms y un máximo de 5\,166 ms. El tamaño medio de las respuestas ha sido de 4\,092 bytes. La tasa de peticiones por segundo (RPS) se ha mantenido en torno a 44,2, mientras que la tasa de errores por segundo ha sido de 7,48. 
    ...
    A pesar de una media de respuesta aceptable, el número significativo de fallos y la elevada dispersión en los tiempos (especialmente el valor máximo y el percentil 95 cercano a 2\,000 ms) sugieren que existen cuellos de botella, especialmente en endpoints críticos como \texttt{checkout}.El percentil 95 se ha usado con el fín de obtener una referencia del rendimiento general del sistema. *Para obtener el tiempo de respuesta promedio se ha sacado siempre de valor de Average(ms).
    ...
    \begin{figure}
        \centering
        \includegraphics[width=1.0\linewidth]{imagen.png}
        \caption{Resumen percentiles Loscust}
        \label{fig:percentiles}
    \end{figure}
    
\end{itemize}
\newpage
\subsubsection{Endpoints críticos} \label{sec:enpoiCri}
...
\begin{itemize}
    ...
    \begin{figure}[h!]
        \centering
        \includegraphics[width=1.0\linewidth]{saliPercent2.png}
        \caption{Salida de medición de locust de los percentiles}
        \label{fig:enter-label}
    \end{figure}
    \newpage
...
\section{Análisis de carga, rendimiento y distribución de recursos}
...
\end{table}
La siguiente tabla muestra información adicional sobre la carga del sistema, la frecuencia de CPU y las operaciones de entrada/salida
\begin{table}[H]
\centering
...
A partir de los datos anteriores, analizamos ahora dos métricas fundamentales en el rendimiento de sistemas: throughput y latencia
\begin{itemize}
...
\subsection{Eficiencia por usuario}
 Es importante evaluar si el sistema mantiene una eficiencia constante a medida que se suman usuarios. 
Para ello calculamos el throughput por usuario activo:
...
\subsection{Análisis de uso de recursos y posibles cuellos de botella}
Una vez determinado que el rendimiento por usuario se mantiene estable, exploramos ahora cómo se comportan los recursos del sistema y como se distribuyen para detectar posibles cuellos de botella
Durante la prueba se han monitorizado los principales recursos del sistema para detectar posibles cuellos de botella. A continuación se detallan las observaciones:

\begin{itemize}
  \item La \textbf{carga promedio del sistema} (\textit{load average}) alcanzó un valor de 1.87 en 1 minuto. Teniendo en cuenta que el sistema cuenta con dos núcleos, esta carga indica que se está acercando al límite de capacidad. Si se continuara aumentando la carga, a partir de los 180 usuarios concurrentes, el sistema podría saturarse, afectando negativamente al rendimiento y a la experiencia del usuario.

  \item La \textbf{CPU} mostró un uso máximo del 37\%. Aunque aún no está saturada, el incremento en la latencia conforme aumenta el número de usuarios sugiere que algunas peticiones empiezan a esperar recursos. Por ahora no representa un problema grave, pero si se incrementa la carga, podría convertirse en un cuello de botella, especialmente considerando que el sistema tiene solo dos núcleos.

  \item La \textbf{RAM} se mantuvo estable durante toda la prueba, sin presentar signos de agotamiento ni de uso excesivo. Por lo tanto, se descarta como factor limitante en este escenario.

  \item La \textbf{latencia} también aumentó ligeramente al acercarse a los 180 usuarios, lo que refuerza la hipótesis de una posible saturación futura de la CPU o del sistema en general.
\end{itemize}

\textbf{En resumen}, aunque en este punto no se ha alcanzado un cuello de botella crítico, el sistema comienza a mostrar señales tempranas de estrés, especialmente en relación con la carga del sistema y el uso de CPU. Si se planea escalar el número de usuarios concurrentes, convendría mejorar los recursos existentes, como por ejemplo aumentar el número de núcleos.

Además del rendimiento, resulta relevante analizar cómo se distribuye el uso de recursos entre las distintas partes del sistema, especialmente cuando hay varios procesos concurrentes. Para ello, se ha calculado el \textbf{índice de equidad de Jain}, que indica cuán equitativamente se distribuyen los recursos del sistema entre los procesos, usuarios o solicitudes. Un valor cercano a 1 implica una distribución equitativa, mientras que valores más bajos indican desigualdad.

\subsubsection*{Cálculo del índice de Jain}
...
\textbf{CPU (\%)}\\
Valores observados: 20.40, 30.30, 36.40, 37.60, 36.40

\[
\sum x_i = 161.1 \qquad \sum x_i^2 = 5397.93
\]
\[
J_{CPU} = \frac{(161.1)^2}{5 \cdot 5397.93} = \frac{25953.21}{26989.65} \approx 0.96
\]

\textbf{RAM (\%)}\\
Valores observados: 35.3, 35.1, 34.7, 34.8, 34.7

\[
\sum x_i = 174.6 \qquad \sum x_i^2 = 6097.32
\]
\[
J_{RAM} = \frac{(174.6)^2}{5 \cdot 6097.32} = \frac{30478.76}{30486.6} \approx 1.00
\]

Estos resultados muestran que el uso de la memoria RAM ha sido prácticamente equitativo entre los procesos concurrentes. En cuanto a la CPU, la distribución también ha sido bastante equilibrada, aunque con ligeras variaciones, lo cual es esperable debido al aumento de carga que está experimentando el sistema. 

Se concluye que, bajo la carga actual de 180 usuarios concurrentes, los recursos del sistema se distribuyen de manera equitativa, sin indicios de desigualdad.

\subsubsection*{Análisis de equidad a partir de percentiles de latencia}
Además del reparto de recursos, es crucial analizar cómo varía la experiencia de los usuarios.
Para ello con los datos obtenidos en el resumen final de Locust, que se pueden consultar en la \autoref{fig:percentiles}, y complementando la información presentada en la sección \autoref{sec:enpoiCri}, podemos recoger los siguientes valores representativos del comportamiento del sistema:

\begin{itemize}
  \item \textbf{Percentil 50 (P50)}: 15 ms
  \item \textbf{Percentil 90 (P90)}: 49 ms
  \item \textbf{Percentil 95 (P95)}: 2000 ms
\end{itemize}

A partir de estos datos, podemos estimar la \textbf{desviación estándar} (\(\sigma\)) utilizando la diferencia entre percentiles:

\[
\sigma \approx \frac{P_{95} - P_{50}}{2} = \frac{2000 - 15}{2} = 992.5 \, \text{ms}
\]

También calculamos la \textbf{media} (\(\mu\)) como el promedio de estos tres percentiles:

\[
\mu \approx \frac{P_{50} + P_{90} + P_{95}}{3} = \frac{15 + 49 + 2000}{3} = \frac{2064}{3} \approx 688 \, \text{ms}
\]

Con estos valores, estimamos el \textbf{coeficiente de variación} (CV), que mide la variabilidad relativa en la latencia:

\[
CV \approx \frac{\sigma}{\mu} = \frac{992.5}{688} \approx 1.44
\]

Un valor de \( CV > 1 \) indica una alta variabilidad y una distribución desigual de los recursos del sistema, lo que sugiere problemas de equidad y posibles cuellos de botella. En sistemas bien optimizados, el CV debería ser menor a 0.5.

De esta manera, podemos confirmar los supuestos planteados anteriormente: aunque el sistema responde rápido para la mayoría de las peticiones (reflejado en el bajo P50 y P90), existe un pequeño porcentaje de solicitudes con latencias muy elevadas (P95). Esto indica que ciertos procesos, como endpoints con una carga de trabajo elevada o tareas intensivas, están acaparando recursos o, en este caso, congestionando el sistema.
...
Según los datos recogidos podemos decir que el sistema matiene un rendimiento más o menos aceptable hasta 180 usuarios, por lo que se puede deducir que el sistema es eficiente debido a que:

\begin{itemize}
    \item El throughput crece bastante mientras la CPU no aumenta de forma proporcional a este.
    \item El sistema aguanta bien la carga sin saturarse ni que la latencia aumente demasiado.
    \item Los recursos del sistema se reparten de manera equitativa.
    \item El percentil 95 nos dice que unas 62 002 peticiones dieron más rapida o iguales a 2000 ms, y 4316 han superado ese umbral. Por lo que hay enpoints que están causando cuellos de botella. Entre estos endpoint destacar el de realizar un pedido ('checkout'), el cual de entre todas sus petiones no fallidas, casi todas las que han superado el umbral son de este enpoint, como se vio con anterioridad.
    \item Se concluye que no todos los usuarios están activos en cada momento, algunos están en espera entre acciones/tareas.
    \item Los errores recogidos han sido errores controlados, por lo que no necesitan ser analizados.
\end{itemize}
...
Por tanto, si se desea escalar el sistema para soportar una carga superior, sería recomendable considerar optimizaciones a nivel de backend o infraestructura, como balanceo de carga, caché de contenido o escalado horizontal mediante la distribución de servicioso un escalado vertical aumentando el número de nucleos.
...
\begin{itemize}
    \item \textbf{Paginación en los endpoints de pedidos}: Para evitar la carga excesiva de datos al listar muchos pedidos. Se muestran 5 pedidos por página y cada vez que se cambia de página se carga unos nuevos pedido, de esta forma, si un usuario realiza muchos pedidos, la consulta a este endpoint no genera tanta carga y lo irá cargando poco a poco todo. Esto se tiene en cuanta a la hora de generar la carga con Locust, de tal forma que se accede a los pedidos de un usuario, se realizan un par de cálculos, y una vez obteneida las páginas, de manera random se accederá a unas o a otras.
...
\begin{itemize}
    \item \textbf{Página web}: Menú principal e implementación del endpoint de pedidos. Creación de sistema de errores con Flask.
