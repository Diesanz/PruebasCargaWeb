...
\section{Introducción Flask}\cite{openwebinars_flask} \cite{flask_docs}
    ...
    \subsection{Uso de entornos virtuales} \cite{venv_python}
...
Para evaluar el rendimiento del sistema, se llevó a cabo una prueba de carga de 25 minutos utilizando Locust\cite{xantygc_locust}. La ejecución se realizó con el siguiente comando:
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item Estas métricas, presentadas en Grafana (apartado 4), permitieron correlacionar rendimiento técnico y experiencia de usuario.
    \end{itemize}
\end{enumerate}

\subsection{Captura periódica y análisis de resultados}
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item Como comprobación se adjunta una captura de los orrores obtenidos en el análisi donde se puede iobservar que son errores contrrolados y que son todos de respuest hhtps de 300 a 308 lo que bos indica que el backend esta funcionando correctamente y que todo va bien como debria ocurrir en casos reales.
        \begin{figure}[h!]
            \centering
            \includegraphics[width=0.7\linewidth]{errores.png}
            \caption{Enter Caption}
            \label{fig:enter-label}
        \end{figure}
    \end{itemize}
...
\noindent A continuación, se incluyen las capturas obtenidas cada 5 minutos, donde se aprecia visualmente la evolución del sistema tanto desde la perspectiva técnica (Grafana) como funcional (Locust).



%ejemplo
\begin{itemize}
    \item \textbf{Captura minuto 0--5:} \\
    ...
    \begin{figure}[h!]
        \centering
        \includegraphics[width=0.5\linewidth]{datos5.png}
        \caption{Enter Caption}
        \label{fig:enter-label}
    \end{figure}
    El sistema respondió con una media de 16,8 peticiones por segundo y un tiempo de respuesta medio de 134,18\,ms, con un percentil 95 del tiempo de respuesta de 2000\,ms. El número total de peticiones fue de 2753. No se registraron errores.
    \begin{figure}[h!]
        \centering
    ...
    \item \textbf{Captura minuto 5--10:} \\
    ...
    El sistema respondió con una media de 35,6 peticiones por segundo y un tiempo de respuesta medio de 132,18\,ms, con un percentil 95 de 2000\,ms. El número total de peticiones fue de 10\,934. No se registraron errores.

    \item \textbf{Captura minuto 10--15:} \\
    ...
    El sistema respondió con una media de 52,8 peticiones por segundo y un tiempo de respuesta medio de 139,58\,ms, con un percentil 95 de 2000\,ms. El número total de peticiones fue de 24\,217. No se registraron errores.

    \item \textbf{Captura minuto 15--20:} \\
    ...
    \begin{figure}[h!]
        \centering
        \includegraphics[width=0.5\linewidth]{datos10.png}
        \caption{Enter Caption}
        \label{fig:enter-label}
    \end{figure}
    
    El sistema respondió con una media de 71,5 peticiones por segundo y un tiempo de respuesta medio de 145,44\,ms, con un percentil 95 de 2000\,ms. El número total de peticiones fue de 43\,246. No se registraron errores.
    \begin{figure}[!h]
        \centering
    ...
    \end{figure}
    \newpage
    \item \textbf{Captura minuto 20--25:} \\
...
\hline
5   & 41  & 134,18 & 16,8 & 20,40 & 35,3 \\
10  & 81  & 132,18 & 35,6 & 30,30 & 35,1 \\
15  & 121 & 139,58 & 52,8 & 36,40 & 34,7 \\
20  & 161 & 145,44 & 71,5 & 37,60 & 34,8 \\
25  & 180 & 149,12 & 79,3 & 36,40 & 34,7 \\
\hline
\end{tabular}
...
\hline
5   & 0,30 & 0,12 & 2095 & 10446 \\
10  & 0,36 & 0,33 & 2095 & 10446 \\
15  & 1,14 & 0,80 & 2095 & 10446 \\
20  & 1,44 & 1,31 & 2095 & 104450 \\
25  & 1,87 & 1,72 & 2095 & 104452 \\
\hline
\end{tabular}
...
\begin{itemize}
  \item La CPU sube hasta un 37\%, pero no llega ni cerca del 100\%, así que no está saturada.
  
  \item La RAM se mantiene bastante estable, sin cambios significativos.
  
  \item La carga promedio del sistema (load average) llega hasta 1.87 en 1 minuto, lo que indica que hay procesos esperando para ejecutarse, pero sigue siendo menos que el número de núcleos (asumiendo al menos 2-4 núcleos).
\end{itemize}

En resumen, la CPU podría ser un posible límite en el futuro, pero todavía hay margen.
...
El throughput por usuario se mantiene casi constante en 0.44 RPS, lo que indica que el sistema escala bien y sigue siendo eficiente por usuario.

\subsection{Posibles cuellos de botella}

\begin{itemize}
  \item La carga del sistema se acerca al número de núcleos (1.87 de load average), por lo que si el servidor tiene 2 núcleos, está casi saturado. Esto podría explicar la ralentización en el crecimiento del throughput.
  
  \item La CPU está al 37\%, no saturada, pero el aumento en latencia indica que algunas peticiones pueden estar esperando recursos. Por ahora no es problema, pero si sube la carga, podría ser un cuello de botella.
  
  \item La RAM se mantiene estable, no parece ser un problema.
\end{itemize}

\subsection{Ley de Little}

La Ley de Little nos dice que:
...
\begin{itemize}
    \item \(N\) = usuarios concurrentes,
    \item \(\mu\) = throughput (peticiones por segundo),
    \item \(R\) = tiempo medio de respuesta en segundos.
\end{itemize}

Calculamos \(\mu \times R\) para ver si encaja con \(N\):

\begin{table}[H]
\centering
\begin{tabular}{|c|c|c|c|}
\hline
\textbf{Tiempo (min)} & \textbf{Usuarios \(N\)} & \textbf{Throughput \(\mu\)} & \textbf{\(X \times R\) (estimado)} \\
\hline
5  & 41  & 16.8  & 2.25 \\
10 & 81  & 35.6  & 4.71 \\
15 & 121 & 52.8  & 7.37 \\
...
\hline
\end{tabular}
\caption{Validación Ley de Little (R en segundos)}
\end{table}

Como \(\mu \times R\) es mucho menor que \(N\), eso significa que no todos los usuarios están activos al mismo tiempo, muchos están en espera. Esto es posible ya que hay un tiempo entre 1 y 3 segundos en los que los usuarios tienen que esperar para realizar las tareas.
...
Pero si aumentamos más usuarios, la CPU podría saturarse y la latencia subir rápido, especialmente porque la carga del sistema ya está cerca del límite.
...
\begin{itemize}
    \item \textbf{Paginación en los endpoints de pedidos}: Para evitar la carga excesiva de datos al listar muchos pedidos. Se muestran 5 pedidos por página y cada vez que se cambia de p+agina se carga unaos nuevos pediso, de esta formasi un usaurio realiza muschos pediso, la consulta a este enpoint no genera tanta carga y lo ira cargando poco a poco todo.
