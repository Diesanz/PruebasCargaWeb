...
\section{Introducción Flask}\cite{openwebinars_flask} \cite{flask_docs}
    ...
    \subsection{Entorno virtual} \cite{venv_python}
...
\section{Contenido y estructura página web} \cite{mdn_mvc}
...
\begin{itemize}
    ...
    \textit{Uso de Jinja2:} se recorre la lista de productos y se renderiza su contenido dinámicamente. También se utiliza `url\_for` para cargar las imágenes estáticas.
    ...
    \textit{Uso de Jinja2:} Se usan bloques condicionales y bucles para renderizar mensajes almacenados mediante Flask Flash.
...
\subsection{Mapa del sitio web}
        \begin{figure}[H]
            \centering
            \includegraphics[width=0.80\linewidth]{./img/mapa_sitio_web.drawio.png}
...
\subsection{Instalación y Configuración de Locust} \cite{locust_video}
...
El archivo \texttt{generar\_carga.py} contiene la definición de un usuario simulado mediante la clase \texttt{MiUsuario}, que ejecuta diversas tareas para representar el comportamiento realista de los usuarios en la tienda. Estas tareas se dividen en varios grupos con diferentes pesos, entre los que destacan:
...
\section{Simulación y Monitorización} \cite{xantygc_locust}
\subsection{Simulación}
Para evaluar el rendimiento del sistema, se realizó una prueba de carga con una duración total de 20 minutos utilizando Locust. 
\begin{lstlisting}[language=bash]
locust -f generar_carga.py  -u 160 -r 0.1333 --run-time 20m --host http://virtual.lab.inf.uva.es:31224
\end{lstlisting}
Durante esta prueba, se simularon 160 usuarios concurrentes ejecutando tareas comunes como el registro, login, consulta de menú, agregado de productos al carrito y checkout. Las tareas fueron ejecutadas con tiempos de espera aleatorios entre 1 y 3 segundos para emular un comportamiento de usuario realista. Por otro lado la tasa de entra de usuarios es de 1 cada 7 segundos más o menos. De esta manera se simulan como van llegando usuarios poco a poco y van generarnado cada vez más carga a lo largo de los 20 minutos.
...
En contextos donde se deseen simular usuarios anónimos accediendo a recursos públicos, estos mismos códigos podrían ser tratados como respuestas válidas (\texttt{success()}). Sin embargo, dado que el objetivo en este caso es evaluar flujos autenticados, se ha optado por marcarlos siempre como \texttt{failure()}.
...
%ejemplo
\begin{itemize}
    \item Captura minuto 0-5:
    Ejemplo: En los primeros 5 minutos, se simularon 50 usuarios concurrentes. El sistema respondió con una media de 120 peticiones por segundo y un tiempo de respuesta medio de 180ms, con un percentil 95 de 250ms. No se registraron errores.

    En Grafana, se observa un aumento progresivo del uso de CPU hasta el 45\%, mientras que la RAM se mantiene estable en torno al 60\%. La carga del sistema permanece por debajo de 1, lo que indica que el servidor aún tiene capacidad disponible.

    ---Captura
    \item Captura minuto 5-10: 
    \item Captura minuto 10-15: 
    \item Captura minuto 15-20: 
\end{itemize}
\subsubsection{Endpoints cŕiticos} Ejemplo
checkout es el endpoint más lento (900 ms) y el que más errores genera (7 %).

    Posible cuello de botella en validación de stock o pasarela de pago.

/carrito también muestra un tiempo alto (650 ms) con 5 % de errores.

    Quizá esté mal optimizada la consulta a la base de datos o el renderizado de productos.

/login es lento (500 ms), y eso afecta a la experiencia de usuario.

    Podría deberse a validaciones, encriptación o gestión de sesión.
    
\subsection{Métricas recogidas}

Durante la prueba de carga, se tomaron manualmente las siguientes métricas cada 5 minutos:

\begin{table}[H]
\centering
\begin{tabular}{|c|c|c|c|c|}
\hline
\textbf{Tiempo (min)} & \textbf{Usuarios activos} & \textbf{Tiempo medio de respuesta (ms)} & \textbf{CPU (\%)} & \textbf{RAM (\%)} \\
\hline
5   &                  &                           &               &              \\
10  &                  &                           &               &              \\
15  &                  &                           &               &              \\
...
\hline
\end{tabular}
\caption{Métricas del sistema recogidas durante la prueba 1}
\end{table}
...
\hline
\textbf{Tiempo (min)} & \textbf{Carga del sistema} & \textbf{Frecuencia CPU} & \textbf{Ope. E/S}} \\
\hline
    5   &                  &                                          &              \\
    10  &                  &                                          &              \\
    15  &                  &                                          &              \\
...
\hline
\end{tabular}
\caption{Métricas del sistema recogidas durante la prueba 2}
\end{table}

\vspace{0.5cm}

\subsection{Cálculo del rendimiento (Throughput)}

Se calcula el \textbf{throughput} (peticiones por segundo) a partir del número de usuarios activos y el tiempo medio de respuesta:

\[
\text{Throughput (req/s)} = \frac{\text{Usuarios activos}}{\text{Tiempo medio de respuesta (segundos)}}
\]

\textit{Nota: convertir la respuesta de milisegundos a segundos dividiendo entre 1000.}

\begin{table}[H]
\centering
\begin{tabular}{|c|c|c|c|}
\hline
\textbf{Tiempo (min)} & \textbf{Usuarios activos} & \textbf{Tiempo medio de respuesta (s)} & \textbf{Throughput (req/s)} \\
\hline
5   &                     &                          &                        \\
10  &                     &                          &                        \\
15  &                     &                          &                        \\
...
\hline
\end{tabular}
\caption{Cálculo del rendimiento del sistema usando throughput}
\end{table}

\vspace{0.5cm}

\subsection{Aplicación de la Ley de Little}

Para validar el comportamiento del sistema, se aplica la \textbf{Ley de Little}:
...
Donde:
\begin{itemize}
    \item $N$: número de usuarios en el sistema.
    \item $X$: throughput (req/s).
    \item $R$: tiempo medio de respuesta (segundos).
\end{itemize}

Se puede aplicar en uno o varios puntos de la prueba para comprobar la consistencia del sistema bajo carga.

\vspace{0.5cm}

\subsection{Métricas adicionales derivadas}

A partir de los datos recogidos por Locust, se pueden calcular métricas adicionales para un análisis más completo:

\begin{itemize}
    \item \textbf{Tasa de error (\%)}: $\frac{\# \text{Fails}}{\# \text{Requests}} \times 100$
    \item \textbf{Éxitos por segundo}: $\text{Current RPS} - \text{Current Failures/s}$
    \item \textbf{Throughput de datos (bytes/s)}: $\text{Average size (bytes)} \times \text{Current RPS}$
    \item \textbf{Análisis de latencia (outliers)}: Comparar media, mediana y percentiles para detectar picos o inestabilidad.
\end{itemize}

\vspace{0.5cm}

\subsection{Eficiencia de uso de recursos}

Se calculan las eficiencias del sistema respecto a los recursos hardware empleados:

\subsubsection*{Eficiencia del uso de CPU}
\[
\text{Eficiencia}_{\text{CPU}} = \frac{\text{Throughput}}{\text{Uso de CPU (\%)}}
\]

\subsubsection*{Eficiencia del uso de RAM}
\[
\text{Eficiencia}_{\text{RAM}} = \frac{\text{Throughput}}{\text{Uso de RAM (\%)}}
\]

\vspace{0.3cm}

\subsubsection*{Índice de saturación relativa}
\[
\text{Índice de saturación relativa} = \frac{\text{Uso de CPU (\%)}}{\text{Uso de RAM (\%)}}
\]

\begin{itemize}
    \item Si el índice es $\gg 1$: la CPU puede ser el cuello de botella.
    \item Si el índice es $\ll 1$: la RAM (o el I/O asociado) podría estar limitando el rendimiento.
\end{itemize}

\vspace{0.5cm}
\subsubsection*{Eficiencia por núcleo}

Relación entre throughput y cantidad de núcleos utilizados:

\[
\text{Eficiencia por núcleo} = \frac{\text{Throughput}}{\text{Nº de núcleos usados}}
\]

Útil para escalar horizontalmente y decidir si añadir más núcleos mejora el rendimiento.

\subsubsection*{Tasa de saturación del sistema}

Relaciona carga del sistema con tiempo de respuesta:

\[
\text{Tasa de saturación} = \frac{\text{Carga del sistema}}{\text{Tiempo medio de respuesta (s)}}
\]

Si crece rápidamente, el sistema no está respondiendo eficientemente al incremento de carga.

\subsection{Análisis de posibles cuellos de botella}

A partir de las métricas anteriores se identifican los siguientes aspectos:

\begin{itemize}
    \item \textbf{Evolución de la latencia}: si el tiempo medio de respuesta aumenta de forma lineal o exponencial.
    \item \textbf{Saturación de recursos}: momento exacto donde CPU o RAM superan ciertos umbrales y cómo afecta al throughput.
    \item \textbf{Disminución del throughput}: identificar si hay descenso pese al aumento de usuarios.
    \item \textbf{Errores registrados}: analizar si aparecen errores como \texttt{ConnectionRefused}, \texttt{Timeouts}, etc., y en qué momento.
\end{itemize}
...
\begin{itemize}
    \item Resumen de los principales cuellos de botella detectados.
    \item Recomendaciones para mitigar el impacto de los problemas identificados, tales como:
    \begin{itemize}
        \item Optimización del código backend.
        \item Escalado horizontal del servidor.
        \item Uso de cachés o balanceadores de carga.
        \item Monitorización continua con alertas en Grafana.
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    \item Número de usurios registrados
    \item Pedidos creados 
    \item Ingresoso totales 
    \item Numero de carritos que tienen itmes dentro 
    \item Top 10 productos más vendidos 
\end{itemize}

\section{Visualización de Grafana}
...
\begin{figure}[!h]
    \centering
    \includegraphics[width=1.1\linewidth]{ingT.png}
...
\begin{figure}[!h]
    \centering
    \includegraphics[width=1.1\linewidth]{productosV.png}
